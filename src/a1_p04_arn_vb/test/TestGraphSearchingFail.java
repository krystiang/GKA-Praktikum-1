...
public class TestGraphSearchingFail {
		...
		try {
			directed =
					(DirectedGraph<AttrVertex, DefaultEdge>) asCounting(
							newDirectedReader().read(Root.pathToSource + "a1_p04_arn_vb/misc/graph4.gka"),
							directedcounts, DirectedGraph.class);
			undirected =
					(UndirectedGraph<AttrVertex, DefaultEdge>) asCounting(
							newUndirectedReader().read(Root.pathToSource + "a1_p04_arn_vb/misc/graph3.gka"),
	...
	public void testBreadth() {
		...
		GraphPath<AttrVertex, DefaultEdge> directedPath =
				directedFinder.apply(directed, new AttrVertex("g"), new AttrVertex("z"));
		...
		GraphPath<AttrVertex, DefaultEdge> undirectedPath =
				undirectedFinder.apply(undirected, new AttrVertex("g"), new AttrVertex("zfail"));
	...
	private void checkPaths(GraphPath<AttrVertex, DefaultEdge> directedPath,
		...
		Collections.addAll(directedVertexList, "");
		List<String> undirectedVertexList = new ArrayList<String>();
		Collections.addAll(undirectedVertexList, "");
